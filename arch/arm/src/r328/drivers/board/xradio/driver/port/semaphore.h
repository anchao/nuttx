...
typedef OS_Semaphore_t xr_drv_sema_t;

#define xr_drv_sema_init(sem, val) OS_SemaphoreCreate(sem, val, XR_DRV_SEMAPHORE_MAX_COUNT)
#define xr_drv_sema_destroy(sem) OS_SemaphoreDelete(sem)
#define xr_drv_sema_release(sem) OS_SemaphoreRelease(sem)

static __always_inline int xr_drv_sema_wait(xr_drv_sema_t *sem, XR_DRV_Time_t ms)
{
	return (OS_SemaphoreWait(sem, ms) == XR_DRV_OK ? 1 : 0);
}
