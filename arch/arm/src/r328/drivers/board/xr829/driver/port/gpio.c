...
void xr_wlan_irq_subscribe(void *func)
{
	...
	hal_gpio_set_pull_state(HAL_GPIO_133, HAL_GPIO_PULL_UP);
	hal_gpio_set_direction(HAL_GPIO_133, HAL_GPIO_DIRECTION_INPUT);	/*pin_x mode */
	hal_gpio_set_driving_level(HAL_GPIO_133, HAL_GPIO_DRIVING_LEVEL2);
	hal_gpio_to_irq(HAL_GPIO_133, &irq_num);
	hal_gpio_irq_request(irq_num, func, HAL_IRQ_TYPE_EDGE_RISING, (void *)NULL);
...
static void wlan_power_gpio_init(int state)
{
	if (state)
		...
		hal_gpio_set_pull_state(HAL_GPIO_134, HAL_GPIO_PULL_DOWN);

	hal_gpio_set_direction(HAL_GPIO_134, HAL_GPIO_DIRECTION_OUTPUT);
	hal_gpio_set_driving_level(HAL_GPIO_134, HAL_GPIO_DRIVING_LEVEL2);
}
