...
#define xr_drv_msgqueue_init(queue, n) OS_MsgQueueCreate(queue, n)
#define xr_drv_msgqueue_destroy(queue) OS_MsgQueueDelete(queue)
#define xr_drv_msgqueue_send(queue, msg) OS_MsgQueueSend(queue, msg, 0)
#define xr_drv_msgqueue_receive(queue, msg) OS_MsgQueueReceive(queue, msg, XR_DRV_WAIT_FOREVER)
...
#define xr_drv_msgqueue_destroy(queue) OS_MsgQueueDelete(queue)
#define xr_drv_msgqueue_send(queue, msg) OS_MsgQueueSend(queue, msg, 0)
#define xr_drv_msgqueue_receive(queue, msg, timeout) OS_MsgQueueReceive(queue, msg, timeout)
