...
static int process_sdio_pending_irqs(struct mmc_host *host)
{
	...
	//if (func) {
		...
		if(!(host->sdio_irq_count%3000))
			SD_LOGI("d1 wait time %lld ns\n",host->sdio_irq_times_ns = HAL_GetTimeNs()- host->start_sdio_irq_times_ns);
#endif
...
static int sdio_irq_thread(void *_host)
{
	...
	do {
		...
		if (host->caps & MMC_CAP_SDIO_IRQ){
			...
			osal_sdmmc_sem_wait(&host->sdio_irq_signal,portMAX_DELAY);
			/*
			while(1){
	...
	osal_sdmmc_thread_delete(&host->sdio_irq_thread);
	return ret;
}
...
//	WARN_ON(!host->claimed);
	...
	if (!host->sdio_irqs++) {
		if (!(host->caps2 & MMC_CAP2_SDIO_IRQ_NOTHREAD)) {
			...
			ret = osal_sdmmc_thread_create(&host->sdio_irq_thread, "ksdioirqd", \
				sdio_irq_thread, host,irq_thread_pri, 16*1024);
			if (ret != OS_OK) {
...
int sdio_claim_irq(struct sdio_func *func, sdio_irq_handler_t *handler)
{
	...
	SD_LOGD("SDIO: Enabling IRQ for %d...\n",func->num);

	if (func->irq_handler) {
		//SD_LOGD("SDIO: IRQ for %s already in use.\n", sdio_func_id(func));
		SD_LOGD("SDIO: IRQ for %d already in use.\n", func->num);
		return -EBUSY;
	}
...
int sdio_release_irq(struct sdio_func *func)
{
	...
	SD_LOGD("SDIO: Disabling IRQ for %s...\n", func->num);
