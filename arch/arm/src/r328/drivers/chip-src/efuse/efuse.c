...
#include <nuttx/irq.h>
#define taskENTER_CRITICAL(x)  do{x=(uint32_t)enter_critical_section();}while(0)
#define taskEXIT_CRITICAL(x)   do{leave_critical_section((irqstate_t)x);}while(0)
...
EFUSE_DEF_ITEM(EFUSE_SSK_NAME, EFUSE_SSK, 128, -1, 13, EFUSE_RO),
EFUSE_DEF_ITEM(EFUSE_OEM_NAME, EFUSE_OEM_PROGRAM, 32, -1, 4, EFUSE_RO),
EFUSE_DEF_ITEM(EFUSE_OEM_SEC_NAME, EFUSE_OEM_PROGRAM_SECURE, 64, -1, 21, EFUSE_RO),
...
{
	return readl(EFUSE_SRAM + key_index);
}
...
{
	return readl(EFUSE_SECURE_MODE) & 0x1;
}
