...
static int usb_gadget_init(struct usb_function_driver *fd)
{
	...
	sem_init(&fd->read_schd, 0, 0);
	sem_init(&fd->write_schd, 0, 0);
	(*fd->desc_init)(fd);
...
int usb_gadget_function_read(int ep_idx, char *buf, int size)
{
	...
	gadget_debug("====\n");
	//while (xSemaphoreTake(fd->read_schd, portMAX_DELAY) != pdPASS) {
...
int usb_gadget_function_write(int ep_idx, char *buf, int size)
{
	...
	//while (xSemaphoreTake(fd->write_schd, portMAX_DELAY) != pdPASS) {
