 ...
 */
static inline void __list_del(struct input_list_head * prev, struct input_list_head * next)
{
	next->prev = prev;
	prev->next = next;
}
 ...
 */
static inline void __list_del_entry(struct input_list_head *entry)
{
	__list_del(entry->prev, entry->next);
}

static inline void list_del(struct input_list_head *entry)
{
	__list_del(entry->prev, entry->next);
	entry->next = NULL;
	entry->prev = NULL;
