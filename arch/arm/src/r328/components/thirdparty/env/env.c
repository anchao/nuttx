...
extern int32_t spinor_program_data(uint32_t addr, const void *buf, uint32_t cnt);
...
static sem_t env_sem;
static int env_lock(void)
{
	int ret = 0;
	ret = sem_init(&env_sem, 0, 1);
	if (ret < 0) {
		printf("create env_sem for lock env failed.\n");
		return -1;
	}

	ret = sem_trywait(&env_sem);
	if (ret < 0) {
...
static int flash_env_read(void *buf, size_t size)
{
	...
	if (dev_current == 0) {
		...
		env = open("/dev/env-redund", O_RDONLY);
		if (env < 0) {
...
{
	int rc = 0;
	rc = spinor_program_data(offset, &obsolete_flag, sizeof(obsolete_flag));
...
static int flash_env_write(FAR const void *buf, size_t size)
{
	int env, env_write, ret = 0;
	if (dev_current == 0) {
		env = open("/dev/env", O_RDWR);
		env_write = open("/dev/env-redund", O_RDWR);
		if (env < 0 || env_write < 0) {
		...
		env = open("/dev/env-redund", O_RDWR);
		env_write = open("/dev/env", O_RDWR);
		if (env < 0 || env_write < 0) {
			printf("get part env, env-redund fail\n");
			close(env);
			close(env_write);
	...
	if ((ret = write(env_write, buf, size)) != size) {
		printf("ret:%d,write env fail\n",ret);
		close(env);
		close(env_write);
		return -1;
	}
...
int fw_printenv(char *arg)
{
	...
	if (arg == NULL) {	/* Print all env variables  */
		char *env, *nxt;
		for (env = environment.data; *env; env = nxt + 1) {
			for (nxt = env; *nxt; ++nxt) {
				if (nxt >= &environment.data[ENV_SIZE]) {
	...
	else {	/* print a subset of env variables */
		char *name = arg;
		char *val = NULL;
