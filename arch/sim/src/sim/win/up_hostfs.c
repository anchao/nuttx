...
static void host_stat_convert(struct _stat *hostbuf, struct nuttx_stat_s *buf)
{
...
int host_open(const char *pathname, int flags, nuttx_mode_t mode)
{
  ...
  int ret = _open(pathname, mapflags, mode);
  if (ret == -1)
    {
      ret = -errno;
    }
...
void *host_opendir(const char *name)
{
  ...
  return (void *)FindFirstFile(namebuf, &data);
}
